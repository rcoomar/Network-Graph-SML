...
import os
import ast
import pathlib  # <-- needed for repo-relative paths
...
def create_network_graph(df):
    ...
    for idx, (_, row) in enumerate(df.iterrows()):
            ...
            for mentioned_user in mentions:
                if mentioned_user and str(mentioned_user) != 'nan' and mentioned_user != author_username:
                    ...
                    if G.has_edge(author_username, mentioned_user):
                        ...
                        G.add_edge(
                            author_username, mentioned_user,
        ...
        try:
            ...
            for ref_tweet in ref_tweets:
                if not isinstance(ref_tweet, dict):
                    continue
                ...
                if cleaned_parent_id in tweet_to_user:
                    ...
                    if pd.isna(target_user) or target_user == author_username or str(target_user) == 'nan':
                        continue
                    ...
                    if G.has_edge(author_username, target_user):
                        ...
                        edge_data = dict(mentions=0, replies=0, retweets=0, quotes=0, total_interactions=1)
                        if ref_type == 'replied_to':
                            edge_data['replies'] = 1
                            interaction_counts['replies'] += 1
                        elif ref_type == 'retweeted':
    ...
    # Totals
    st.success("**Interaction Statistics:**")
...
def plot_network_graph(G, layout='spring', max_nodes=None):
    ...
    # Edges
    edge_x, edge_y, edge_text, edge_width, edge_colors = [], [], [], [], []
    for edge in G.edges():
        try:
            source, target = edge
            x0, y0 = pos[source]
            ...
            edge_text.append(
                f"From: @{source}<br>"
                f"To: @{target}<br>"
                f"Total Interactions: {total_weight}<br>" + "<br>".join(interactions)
            )
        ...
        for i in range(0, len(edge_x), 3):
            if i + 2 < len(edge_x):
                fig.add_trace(go.Scatter(
                    ...
                    line=dict(width=edge_width[i//3], color=edge_colors[i//3]),
                    hoverinfo='text',
    ...
    # Fixed title styling (no titlefont)
    fig.update_layout(
        title=dict(
            text='Twitter User Interaction Network'
                 '<br><sub>Node size: Followers | Node color: User type (Gray: Not in dataset) | Edge width: Interaction frequency</sub>',
            font=dict(size=16)
        ),
        showlegend=False,
        hovermode='closest',
...
def main():
    ...
    candidates = [
        ...
        app_dir / "data" / file_basename,     # data subfolder
        app_dir.parent / file_basename,       # one level up
    ]

    file_path = None
    for cand in candidates:
        if pathlib.Path(cand).exists():
            file_path = pathlib.Path(cand)
            break
    ...
    min_followers = st.sidebar.slider(
        ...
        min_value=0, max_value=int(df['followers_count'].max()), value=0,
